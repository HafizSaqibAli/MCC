...
#ifndef UPDATE_FRAGMENT_SHELL_TYPE_FORCES
#define UPDATE_FRAGMENT_SHELL_TYPE_FORCES
		...
		if(checki){			
			...
			for (int j = 0; j < molecules[i].getSize(); j++){
				...
				for (int t = 0; t < fragmentShellFragmentTypes.size(); t++){
					...
					if (molecules[i].returnFragment(j) == fragmentShellFragmentTypes[t] && molecules[i].returnFragmentShell(j) == fragmentShellShellTypes[t]){
							inTypes=true;
							fragmentShellFragmentTypes[t].incrementOccurence();
							fragmentShellFragmentTypes[t].addForceAverage(molecules[i].getFragment(j)->getForceAverage());
							fragmentShellFragmentTypes[t].addForceSquared(molecules[i].getFragment(j)->getForceSquared());
					}
				}

				if (inTypes == false){
					fragmentShellShellTypes.push_back(molecules[i].returnFragmentShell(j));
					fragmentShellFragmentTypes.push_back(molecules[i].returnFragment(j));
					fragmentShellFragmentTypes[fragmentShellFragmentTypes.size() - 1].addForceAverage(molecules[i].getFragment(j)->getForceAverage());
					fragmentShellFragmentTypes[fragmentShellFragmentTypes.size() - 1].addForceSquared(molecules[i].getFragment(j)->getForceSquared());
