function [ ] = RADSpaceGraphs()
	...
	while t <= length(tempstr)
		...
		Filename = [char(tempstr(t)) '/oldfcmatrix.txt'];
...
if (( t == 0 ) || (t == 0) )
		...
		splotpos(3) = splotpos(3)*1.05; % Take 20 percent off width (3);
                if t > 7
	        ...
	        set(splot, 'position', splotpos);
                ...
                colormap(gray(50));
%colormap(hot);
                yshift = m - 25; % Shift plot up to top
		xshift = 0 - 0.5*(25-m);
                x2shift = 25+xshift;
...
axis off;
	...
	outputFilename = ['offset_fcmatrix.eps'];
%	set(gcf, 'PaperPositionMode', 'auto');
	print('-depsc', outputFilename);
	print('-djpeg', 'fcmatrix.jpg');
        print('-dtiff', '-r355', 'fcmatrix.tif');
